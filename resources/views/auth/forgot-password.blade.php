<x-guest-layout>
    ...
    <form method="POST" action="{{ route('password.email') }}" class="space-y-5">
        ...
        <!-- Security Question -->
        <div>
            <x-input-label for="security_answer" :value="__('auth.security_answer')" class="text-sm font-semibold text-gray-700" />
            <x-text-input 
                id="security_answer" 
                ...
                placeholder="{{ __('auth.enter_answer') }}"
                required 
                autofocus 
            />
